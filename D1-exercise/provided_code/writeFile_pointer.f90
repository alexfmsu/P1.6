...
PROGRAM write_file
 USE MPI
 IMPLICIT NONE
 INTEGER :: err, i, myid, file, intsize, size, offset
 ...
 offset = count / size
  ...
  disp= myid* offset * intsize

  CALL MPI_FILE_SEEK(file, disp, MPI_SEEK_SET, err)
  CALL MPI_FILE_WRITE(file, buf(myid * offset), offset, MPI_INTEGER, status, err)
